...
| 小程序端 | 微信原生小程序（wxml/wxss/js，目录 `mini_app/`） |
| 存储 | SQLite（单文件双库） |
...
| 通信 | 桌面端局域网 HTTP（Web 8780 / 手机联动 8781），小程序走 8780 |
...
| **待办任务** | 采集信息经 AI 提炼出的待办任务；支持完成/忽略/恢复/删除，双击=完成 |
| **信息采集** | 手动粘贴/输入文本提交入库；下方信息库列表 |
| **手机联动号码** | 为 6 个打电话动作配置目标号码，保存后生效 |
| **设置** | 18 项关键配置（见「八、设置项说明」） |
| **运行状态** | 各服务/进程/数据统计/最近日志实时查看 |
| **动作日志** | 所有动作执行记录（成功/失败/消息） |

> 桌面右侧还有**待办任务实况窗**（置顶）：未到期点击=暂时忽略，到截止日期标红「▲ 已到期」，点击=完成。

### 5.3 常用操作
1. **新建会议**：会议准备页 → 新建会议 → 填主题/时间 → 创建并开始准备 → 一阶段自动并行启动
2. **人工确认**：对 `waiting` 项点「确认」→ 标记完成，二阶段自动推进
3. **配号码**：手机联动号码页 → 填号码 → 保存
4. **录信息**：信息采集页 → 粘贴文本 → 提交（交 AI 提炼，同时产出会议要点与待办任务）
5. **管理任务**：待办任务页或右侧实况窗处理待办
6. **改配置**：设置页 → 修改 → 保存设置

### 5.4 材料 / PPT / 印刷（双线操作）
- **AI 自动**（新建会议勾选「AI 自动生成材料并发送」）：审批通过后自动生成材料 → 自绘 PPT → md→PDF → 微信发印刷 → 电话安排印刷（**份数=与会人数+5**）
- **人工操作**（未勾选）：审批后材料+文件已生成，到会议详情的「**材料/文件**」区：
  - 材料 JSON **可直接编辑**（AI 生成后自由修改）→「保存材料」
  - 「生成 PPT/PDF」基于编辑后内容重新生成
  - 文件列表可**指定发送**任意文件（PDF/PPT/MD）给印刷微信联系人
- 文件按会议分组存放在 `data\out\{会议id}_{会议名}\`

### 5.5 智能采集（AI + 模拟操作 + OCR）
- 设置页开启「智能采集」+ 填写**重点识别对象昵称**（逗号分隔）与**角色上下文**（默认技术科科长）
- 保持目标应用（如微信）**在前台** → 桌面「运行状态」页点「立即采集」或等自动轮询
- 系统自动：定位窗口 → 读会话列表 → AI 筛选（重点对象→技术科相关）→ 模拟点击各会话 → OCR 读取 → 提炼入库（会议要点+待办任务）→ 单轮自动停
- 「停止」按钮可随时中断
...
小程序是工作台的「手机轻操控」端：手机上看会议进度、处理待办、录信息、扫码签到，不用开电脑。

### 8.1 功能一览

| 页面 | 功能 |
|------|------|
| 首页 | 最近会议列表（含一/二阶段完成进度条）、进行中待办数 |
| 会议详情 | 一/二阶段动作项状态；`waiting` 项点「确认完成」；「扫码签到」+ 签到名单；重跑一阶段 |
| 任务 | 待办列表，支持完成 / 忽略 / 恢复，按状态筛选 |
| 信息 | 手动录文本提交（AI 提炼）；「扫一扫」扫码内容可编辑后入库 |
| 我的 | 服务器地址 / Token 配置与连通性测试；生成电脑端访问二维码 |

### 8.2 后端配套（已内置）

- **Token 鉴权**：设置项「小程序 Token」（`config.json` 的 `mini.token`），请求头 `X-API-Token`；留空则不校验（仅限局域网调试）。所有 `/api/*` 生效，`/api/health` 免鉴权用于连通测试。
- **局域网监听**：Web 端口默认监听 `0.0.0.0`，手机同 WiFi 可直接访问。
- **聚合接口**：`GET /api/mini/overview` 一次返回首页所需数据。
- **扫码签到**：`GET/POST /api/meetings/{id}/attendance`，同名去重，数据在 `meeting_attendance` 表，桌面/Web 端可直接查看。
- **二维码生成**：`GET /api/mini/qr`（带 token）用 qrcode 生成，默认内容为电脑端 Web 地址（自动探测局域网 IP），返回 base64 PNG。

### 8.3 联调步骤（开发版，无需备案）

1. 电脑端启动工作台（`python main.py`），确认 Web 端口 8780 正常。
2. Windows 防火墙放行 **8780**（TCP 入站，仅信任的局域网）。
3. 微信开发者工具「导入项目」选择 `mini_app/`，AppID 用测试号，详情勾选「不校验合法域名」。
4. 手机与电脑连同一 WiFi，点「真机预览」，手机微信打开后到「我的」页：
   - 服务器地址填 `http://电脑局域网IP:8780`（电脑 `ipconfig` 查看）
   - 访问口令填电脑端设置页的「小程序 Token」
   - 点「测试连接」，显示「连接成功」即可用。

### 8.4 发布正式版（以后可做）

正式版要求服务器域名 **HTTPS + ICP 备案**（如 Cloudflare Tunnel / frp 内网穿透 + 备案域名），并在小程序后台配置 request 合法域名；个人主体注册需成人实名，可等成年后再提审。
...
| 小程序 Token | 密码 | 空 | 小程序访问口令（请求头 X-API-Token）；留空=不校验（仅局域网调试） |
| 采集轮询间隔(秒) | 整数 | 300 | 微信等来源扫描频率 |
| 启用微信采集 | 布尔 | 开 | 是否用 UIA 扫微信会话 |
| 启用钉钉采集 | 布尔 | 关 | 是否读取钉钉会话（需钉钉登录打开） |
| 启用飞书采集 | 布尔 | 关 | 是否读取飞书会话（需飞书登录打开） |
| 启用OneNote采集 | 布尔 | 关 | 是否读取 OneNote 页面（需打开） |
...
| 智能采集 | 布尔 | 关 | AI+模拟操作自动遍历聊天会话采集（默认关） |
| 智能采集应用 | 文本 | wechat | wechat / dingtalk / feishu |
| 智能采集间隔(秒) | 整数 | 300 | 自动采集轮询频率 |
| 每轮最多采集会话数 | 整数 | 5 | 限制模拟点击会话数 |
| 重点识别对象昵称 | 文本 | 空 | 逗号分隔的昵称/群名，采集优先识别 |
| 角色上下文 | 文本 | 技术科科长 | AI 识别聚焦的角色 |
...
├── mini_app/                  # 微信小程序端（原生 wxml/wxss/js）
├── phone_app/                 # 鸿蒙手机端工程
├── docs/                      # 文档
├── build/                     # 打包中间产物
├── dist/                      # 发布产物
...
**Q11：小程序连不上电脑端？**
按 8.3 逐项检查：1. 工作台已启动；2. 手机与电脑同一 WiFi；3. Windows 防火墙放行 8780；4. 「我的」页地址格式为 `http://IP:8780`（末尾不带斜杠）；5. Token 与电脑端设置一致（服务端开启校验时）；6. 开发者工具勾选「不校验合法域名」。

**Q12：扫码签到重复怎么办？**
同名签到自动去重（返回「已签过到」），不会产生重复记录；签到数据在 `meeting_attendance` 表，桌面/Web 端可查。

**Q13：生成的电脑端二维码是哪个地址？**
后端自动探测电脑局域网 IP（`/api/mini/qr`），手机扫开即电脑端 Web 界面；换了 WiFi 网段 IP 变化时重新生成即可。
